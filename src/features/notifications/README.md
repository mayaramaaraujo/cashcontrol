...
- `components/NotificationToggle.tsx` — the Settings page's "Push notifications" row (reuses `shared/components/Switch`). Requests `Notification.requestPermission()` on enable, subscribes, and persists the subscription; unsubscribes and deletes on disable.
