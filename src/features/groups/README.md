...
- **`components/CurrencySwitcher.tsx`** — `SegmentedControl` over `CURRENCIES` (`src/shared/lib/currency.ts`), calling `updateGroupCurrency` in a transition. Shown on the People tab, since currency is a group-wide setting (income/bills are shared), not a per-user preference like language.
