...
- **`lib.ts`** — plain functions computing everything from server-fetched data: `computeHero` (three summary modes), `computeMemberStrip` (per-member monthly totals), `buildIncomeActivity`/`buildBillActivity`/`mergeActivity` (the merged, date-sorted activity feed). No React state — the source data only changes on a full page refetch.
- **`components/HeroSection.tsx`** — client component wrapping just the `SegmentedControl` + hero card (local `summaryMode` state).
