...
- `lib.ts` — plain functions: `lastSixMonths`/`monthLabel` (date helpers, reused by the Bills page for its own category breakdown), `computeTrend` (per-month totals + bar heights relative to the 6-month max), `computeCategoryBreakdown` (current month's income grouped by category, sorted descending, returns `CategoryBreakdownRow[]` from `@/shared/components/CategoryBreakdown`), `earlierMonths` (the trend's non-current months, most recent first, year-qualified labels).
